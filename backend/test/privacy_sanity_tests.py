...
class TestPrivacySanity:
    ...
    def test_national_id_minimization_ordering(self):
        ...
        """
        ascending_ordered_national_ids = sorted(
            [TestPrivacySanity.generate_random_national_id() for _ in range(10)]
        )

        # Preserves the order
        obfuscated_national_ids = [
            Voter("Some", "Voter", national_id)
            .get_minimal_voter()
        ...
        descending_ordered_obfuscated_national_ids = reversed(
            sorted(obfuscated_national_ids)
        )
    ...
    def test_ballot_number_ordering(self):
        ...
        for _ in range(10):
            ...
            assert (
                ballot_numbers_in_creation_order != ballot_numbers_in_descending_order
            )
