...
class TestRegistry:
    ...
    def test_voter_registration(self):
        ...
        for voter in all_voters:
            assert (
                registry.get_voter_status(voter.national_id)
                == VoterStatus.NOT_REGISTERED
            )
            assert registry.register_voter(voter), "Couldn't register {0} {1}".format(
    ...
    def test_different_format_national_id(self):
        ...
        assert (
            registry.get_voter_status(voter.national_id) == VoterStatus.NOT_REGISTERED
        )
        assert (
            registry.get_voter_status(same_voter.national_id)
            == VoterStatus.NOT_REGISTERED
        )
        assert registry.register_voter(voter), "Couldn't register {0} {1}".format(
            voter.first_name, voter.last_name
        ...
        assert (
            registry.register_voter(same_voter) is False
        ), "The exact same voter was recognized as a different voter".format(
            voter.first_name, voter.last_name
        )
        assert (
            registry.get_voter_status(voter.national_id)
    ...
    def test_de_register_voter(self):
        ...
        assert registry.de_register_voter(
            voter.national_id
        ), "Couldn't de-register {0} {1}".format(voter.first_name, voter.last_name)
        assert (
            registry.get_voter_status(voter.national_id) == VoterStatus.NOT_REGISTERED
        )
