...
from main.objects.voter import BallotStatus, Voter, VoterStatus
...
class TestBalloting:
    ...
    def test_count_ballot(self):
        ...
        assert (
            registry.get_voter_status(voter1.national_id) == VoterStatus.BALLOT_COUNTED
        )
        assert (
            registry.get_voter_status(voter2.national_id) == VoterStatus.BALLOT_COUNTED
        )
        assert (
            registry.get_voter_status(voter3.national_id) == VoterStatus.BALLOT_COUNTED
        )
    ...
    def test_ballot_comment_redaction_composite(self):
        ...
        assert (
            balloting.count_ballot(ballot, voter.national_id)
            == BallotStatus.BALLOT_COUNTED
        )
        ...
        Id: {2}
        """.format(
            "[REDACTED NAME]", "[REDACTED NAME]", "[REDACTED NATIONAL ID]"
        ).strip()
    ...
    def test_catch_fraud(self):
        ...
        ballot2 = Ballot(
            ballot_number2, all_candidates[0].candidate_id, "second ballot"
        )
        ballot3 = Ballot(ballot_number3, all_candidates[0].candidate_id, "third ballot")

        # Have the voter commit fraud
        assert (
            balloting.count_ballot(ballot1, voter.national_id)
            == BallotStatus.BALLOT_COUNTED
        )
        assert (
            balloting.count_ballot(ballot2, voter.national_id)
            == BallotStatus.FRAUD_COMMITTED
        )
        assert (
            registry.get_voter_status(voter.national_id) == VoterStatus.FRAUD_COMMITTED
        )
        assert (
            balloting.count_ballot(ballot3, voter.national_id)
            == BallotStatus.FRAUD_COMMITTED
        )
        assert (
            registry.get_voter_status(voter.national_id) == VoterStatus.FRAUD_COMMITTED
        ...
        ) in balloting.get_all_fraudulent_voters()
    ...
    def test_invalidate_ballot_after_use(self):
        ...
        ballot1 = Ballot(
            ballot_number1,
            all_candidates[0].candidate_id,
            "valid now, but invalid later",
        )
    ...
    def test_count_ballot_mismatch(self):
        ...
        ballot2_attempt1 = Ballot(
            ballot_number2, all_candidates[0].candidate_id, "Attempt 1"
        )
        assert (
            balloting.count_ballot(ballot2_attempt1, voter1.national_id)
            == BallotStatus.VOTER_BALLOT_MISMATCH
        )

        # Now, have the right voter cast the ballot
        ballot2_attempt2 = Ballot(
            ballot_number2, all_candidates[1].candidate_id, "Attempt 2"
        )
        assert (
            balloting.count_ballot(ballot2_attempt2, voter2.national_id)
            == BallotStatus.BALLOT_COUNTED
        )
    ...
    def test_unregistered_voter_count_ballot(self):
        ...
        valid_ballot = Ballot(
            valid_ballot_number, all_candidates[0].candidate_id, "Valid Ballot"
        )
        assert (
            balloting.count_ballot(valid_ballot, unregistered_voter.national_id)
